...
Why rewrote the Airbnb's software ? Firt of all, well, we're not as easy as it seems in Ruby! And, we need to add new features to the tool. 2 choices: learning Ruby, and propose PR, or rewrote in a language we know. We choose the second option.
...
* `heartbeat_path`: a path to a file on disk to touch as nerve makes progress.
