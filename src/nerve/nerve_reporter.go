...
type ReporterI interface {
	Initialize(IP string, Port int, Rise int, Fall int, Weight int) error
	Report(Status int) error
	Destroy() error
	GetType() string
}

func CreateReporter(IP string, Port int, ServiceName string, config NerveReporterConfiguration, ipv6 bool) (reporter ReporterI, err error) {
	...
	reporter.Initialize(IP,Port,config.Rise,config.Fall,config.Weight)
	return reporter, nil
}
